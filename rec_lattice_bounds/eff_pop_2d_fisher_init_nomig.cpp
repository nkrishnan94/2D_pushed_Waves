...
int getFrontDeme(long double arr[n_demesh][n_demesw][n_spec], int L){
	...
	for(int j =0 ; j< L; j++){
		...
		for(int i = 1; i < n_demesh; i++){
			if( (arr[i][int(n_demesw/2)- int(L/2)+j][0]+arr[i][int(n_demesw/2)- int(L/2)+j][1]>(.5*K)) &&(frontFound==0)){
				fronts[j]=i;
...
int main (int argc, char * argv[]){
	...
	/*vector <double> rough_histe_1;
	vector <double> rough_histe_2;
	...
	vector <double> rough_histf_1;
	vector <double> rough_histf_2;
	...
	//data files
	ofstream flog, fpop, fhet, fprof,fsects,frough_1,frough_2,frough_4,frough_8,frough_16,frough_32,frough_64;
	time_t time_start;
	...
	/*string rough1Name = "rough_1_" + param_string + date_time.str() + ".txt";
	string rough2Name = "rough_2_" + param_string + date_time.str() + ".txt";
	...
	string folder = "column_migration/sim_data_1D/";
	//string folder = "";
    ...
    /*frough_1.open(folder+rough1Name);
    frough_2.open(folder+rough2Name);
	...
	//while(ht>h_thresh){
		...
		for( int j=0; j<n_demesw; j++){
			...
			for(int ne = 0; ne <1; ne++){
				...
				f1+= (M/2)*deme_aux[neighbs[ne][0]][neighbs[ne][1]][0]/int(K);
				f2+= (M/2)*deme_aux[neighbs[ne][0]][neighbs[ne][1]][1]/int(K);
		...
		if (dt % record_time == 0){
			...
			/*rough_histe_1.push_back(getEdgeDeme(deme,1));
			rough_histe_2.push_back(getEdgeDeme(deme,2));
			...
			rough_histf_1.push_back(getFrontDeme(deme,1));
			rough_histf_2.push_back(getFrontDeme(deme,2));
   ...
   for(int i=0;i < n_data;i++){
    	...
    	/*frough_1<< int(i*record_time) << ", "  << rough_histf_1[i]<< ", "  << rough_histe_1[i] << endl;
    	frough_2<< int(i*record_time) << ", "  << rough_histf_2[i]<< ", "  << rough_histe_2[i] << endl;
