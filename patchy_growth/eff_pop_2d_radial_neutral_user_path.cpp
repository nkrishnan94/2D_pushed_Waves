...
long double deme_g[n_demes][n_demes] = {{0}};
...
int main (int argc, char * argv[]){
	...
	//data files
	ofstream flog, fpop, fhet, fprof, fsect,fmut,ffull,fpatch;
	time_t time_start;
	...
	ostringstream date_time, Kstr,  Mstr, Bstr, Gstr, Istr, Pstr,PNstr;
	date_time << buffer;
	Kstr << K;
	Mstr << M;
	Bstr << B;
	Istr << initMut;
	Gstr << g0;
	string param_string =  "K"+Kstr.str()+"_M" + Mstr.str() + "_B" +Bstr.str() + "_G" +Gstr.str() +"_I"+Istr.str()+"_P"+Pstr.str()+"_PN"+PNstr.str()+"_";



	string logName = "log_cust" + param_string + date_time.str() + ".txt";
	string hetName = "het_cust" + param_string +  date_time.str() + ".txt";
	string varhetName = "varhet_cust" + param_string +  date_time.str() + ".txt";
	string popName = "pop_cust"+ param_string +  date_time.str() + ".txt";
	string profName = "prof_cust" + param_string + date_time.str() + ".txt";
	string patchName = "patch_cust_" + param_string + date_time.str() + ".txt";
	string sectName = "sect_cust" + param_string + date_time.str() + ".txt";
	string mutName = "mut_cust" + param_string + date_time.str() + ".txt";
	string fullName = "full_cust" + param_string + date_time.str() + ".txt";
    ...
    for (int i =3; i<8; i++){
    	patchc[i][0] = 200+(i-2)*patch_size;
    	patchc[i][1] = 150 -2*patch_size ;
    }
    for (int i =8; i<16; i++){
    	patchc[i][0] = 200+5*patch_size;
    	patchc[i][1] = 150-2*patch_size+(i-8)*patch_size;

    }
    for (int i =16; i<21; i++){
    	patchc[i][0] = 200+(i-15)*patch_size;
    	patchc[i][1] = 150+(5)*patch_size;

    }

    for (int i =21; i<24; i++){
    	patchc[i][0] = 200;
    	patchc[i][1] = 150+3*patch_size+ (i -21)*patch_size;
    ...
    //int patchc[2][11] = { {int(n_demes/2), int(n_demes/2)+ 1*patch_size, int(n_demes/2)+ 2*patch_size, int(n_demes/2)+ 3*patch_size,int(n_demes/2)+ 4*patch_size, int(n_demes/2)+ 4*patch_size,int(n_demes/2)+ 4*patch_size,int(n_demes/2)+ 3*patch_size,int(n_demes/2)+ 2*patch_size,int(n_demes/2)+ 1*patch_size,int(n_demes/2)}, {int(n_demes/2),int(n_demes/2),int(n_demes/2),int(n_demes/2),int(n_demes/2),int(n_demes/2)+1*patch_size,int(n_demes/2)+2*patch_size,int(n_demes/2)+2*patch_size,int(n_demes/2)+2*patch_size,int(n_demes/2)+2*patch_size,int(n_demes/2)+2*patch_size,}};
    for(int p=0; p<24; p++){
    	//int p_d1 = gsl_ran_flat(r, 0,n_demes); 
    	//int p_d2 = gsl_ran_flat(r, 0,n_demes); 
    	int p_d1 =patchc[p][0];
    	int p_d2 = patchc[p][1];
    	for(int i =p_d1; i<p_d1+patch_size; i++){
    		for(int j =p_d2; j<p_d2+patch_size;j++){
    			if ((i<n_demes) && (j<n_demes)){
    				deme_g[i][j]=g0;
    			} else{
    				deme_g[i][j]=0;
    			}
	...
	for (int dt = 0 ; dt < n_gens; dt++ ){
		...
		/*	int sectCounts = 0;
	        ...
	        if (prof_hist !=0){
	        	ostringstream strT;
	        	strT << dt;
	        	string proftName = "prof_cust_T"+ strT.str() + "_" + date_time.str() + ".txt";
	        	ofstream fproft;
