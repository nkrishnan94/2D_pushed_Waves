...
float calcRoughness(long double arr[n_demesh][n_demesw][n_spec], int ind ){

	long double freqs[n_demesh];
	float sumFreq=0;
	float sumDiffs=0;
	int j=ind;
	...
	for(int i=0;i<n_demesh;i++){
		...
		freqs[i] = arr[i][j][0]/ (arr[i][j][0]+arr[i][j][1]);
		sumFreq+=arr[i][j][0]/ (arr[i][j][0]+arr[i][j][1]);
	}

	for(int i=0;i<n_demesh;i++){
		sumDiffs+= abs(freqs[i] - sumFreq/n_demesh);

	}

	return sumDiffs/n_demesh;

}
int calcBoundPos (long double arr[n_demesh][n_demesw][n_spec], int ind ){
	long double freqs[n_demesh];
	float sumFreq=0;
	int boundDeme=0;
...
int calcBoundWidth (long double arr[n_demesh][n_demesw][n_spec], int ind ){
	long double freqs[n_demesh];
	int boundDeme=0;
	...
	//for(int i=1;i<n_demesw;i++){
		if ((start==0) && (freqs[i]!=1) && (freqs[i-1]==1) ) {
			start = i;
		...
		if ((end==0) &&(start!=0)&&(freqs[i]==0) && (freqs[i-1]!=0)) {
			end = i;
...
int main (int argc, char * argv[]){
	...
	vector <double> rough_5;
	vector <double> rough_10;
	vector <double> rough_15;
	vector <double> rough_20;
	vector <double> rough_25;
	vector <double> rough_30;
	vector <double> rough_35;
	vector <double> pos_5;
	vector <double> pos_10;
	vector <double> pos_15;
	vector <double> pos_20;
	vector <double> pos_25;
	vector <double> pos_30;
	vector <double> pos_35;
	...
	for(int i = 0; i < int(n_demesh/2); i++){
		for(int j = 0; j < int(n_demesw); j++){
	...
	for(int i = int(n_demesh/2) ; i < n_demesh; i++){
		for(int j = 0; j < n_demesw; j++){
	...
	//while(ht>h_thresh){
		...
		if (dt % record_time == 0){
	        ...
	        rough_5.push_back(calcRoughness(deme,5));
	        rough_10.push_back(calcRoughness(deme,10));
	        rough_15.push_back(calcRoughness(deme,15));
	        rough_20.push_back(calcRoughness(deme,20));
	        rough_25.push_back(calcRoughness(deme,25));
	        rough_30.push_back(calcRoughness(deme,30));
	        rough_35.push_back(calcRoughness(deme,35));
	        pos_5.push_back(calcBoundWidth(deme,5));
	        pos_10.push_back(calcBoundWidth(deme,10));
	        pos_15.push_back(calcBoundWidth(deme,15));
	        pos_20.push_back(calcBoundWidth(deme,20));
	        pos_25.push_back(calcBoundWidth(deme,25));
	        pos_30.push_back(calcBoundWidth(deme,30));
	        pos_35.push_back(calcBoundWidth(deme,35));
   ...
   for(int i=0;i < n_data;i++){
    	...
    	frough << int(i*record_time) << ", " << rough_5[i] << ", "<< rough_10[i] << ", "<< rough_15[i]<< ", " << rough_20[i]<< ", " << rough_25[i]<< ", " << rough_30[i]<<", " << rough_35[i]<<endl;
    	fpos << int(i*record_time) << ", " << pos_5[i]<<", " << pos_10[i] << ", "<< pos_15[i] << ", "<< pos_20[i]<< ", " << pos_25[i]<< ", " << pos_30[i]<< ", " << pos_35[i]<<endl;
    }
