...
int neighborMatch(long double arrNum[][n_demes][n_spec], long double arrGrid[][n_demes], int x, int y){
	...
	if ((arrGrid[x][y]==-1) && ((arrNum[x][y][1]/ (arrNum[x][y][0]+arrNum[x][y][1]))  > .5 ) ){
		arrGrid[x][y]=0;

		if (arrNum[x-1][y][1]/ (arrNum[x-1][y][0]+arrNum[x-1][y][1]) >.5){
		...
		if (arrNum[x][y-1][1]/ (arrNum[x][y-1][0]+arrNum[x][y-1][1]) >.5){
...
int main (int argc, char * argv[]){
    ...
    while ((c = getopt (argc, argv, "K:Z:B:M:U:G:F")) != -1)
    {
        if (c == 'K')
            ...
            innocRad = atoi(optarg); // migration probability
        else if (c == 'G')
	...
	ostringstream date_time, Kstr,  Mstr, Bstr, Gstr, Istr;
	date_time << buffer;
	Kstr << K;
	Mstr << M;
	Bstr << B;
	Istr << innocRad;
	Gstr << g0;
	string param_string =  "K"+Kstr.str()+"_M" + Mstr.str() + "_B" +Bstr.str() + "_G" +Gstr.str() +"_I"+Istr.str()+"_";
	...
	for(int i = int(n_demes*.5)-innocRad; i < int(n_demes*.5)+innocRad; i++){
		for(int j = int(n_demes*.5)-innocRad; j < int(n_demes*.5)+innocRad; j++){
			if (sqrt(abs(i-int(n_demes*.5))*abs(i-int(n_demes*.5)) + abs(j-int(n_demes*.5))*abs(j-int(n_demes*.5))) < innocRad)
			{
				deme[i][j][1] = initMut;
